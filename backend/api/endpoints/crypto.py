...
async def encrypt_message(
    ...
    try:
        ...
        if not crypto_result.success:
            raise HTTPException(
                status_code=400,
                detail=crypto_result.error_message or "Encryption failed"
            )
...
async def decrypt_message(
    ...
    try:
        ...
        if not crypto_result.success:
            raise HTTPException(
                status_code=400,
                detail=crypto_result.error_message or "Decryption failed"
            )
